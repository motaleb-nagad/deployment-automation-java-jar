...
| **Deploy** | Build a `./run.sh`-equivalent (group → hosts → apps → actions), confirm the blast radius, then stream the run live per-host. Includes a **Ready to deploy** dashboard — the jars staged in `files/jars/` with their git hash (as `hash-check.sh` shows) compared against what production currently runs (real hash via `check-hash.yml`). |

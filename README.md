...
| **Promote** | Fetch a built jar from staging → reads its **real git commit hash** (from the jar's `git.properties`, matching `hash-check.sh`), records it to `registry-db`. Two-step: review the hash first, then confirm. Never touches production. |
| **Deploy** | Build a `./run.sh`-equivalent (group → hosts → apps → actions), confirm the blast radius, then stream the run live per-host. Includes a **Ready to deploy** dashboard — the jars staged in `files/jars/` with their git hash (as `hash-check.sh` shows) compared against what production currently runs (real hash via `check-hash.yml`). After any **stop/start** run the console runs an independent, read-only **process-table check** on each host (`ps -u <service-user>` for the app's jar, polled up to a timeout) and reports the real live state — `running` / `stopped` / `not-running` / `still-running` / `unverified` — instead of trusting Ansible's "started" line; a service that didn't reach its intended state is flagged as an **incident** with a one-click **retry**. See [`ops/ansible/README-verify-start-stop.md`](ops/ansible/README-verify-start-stop.md). |
| **Stg Deployment** | The **staging** channel (`/stg-deployment`), backed by a wholly separate backend **container** (`deployment-automation-backend-stg`). Upload-driven deploy, plus two sub-views under the same tab: **Application Properties** (surgical `application.properties` edits on the staging host) and **History** (every staging run / portal-UI deploy / property edit). The review shows each file's **SHA-256** (computed in the browser) before anything is uploaded. |
| **Application Properties** | Surgical edits of `application.properties` for one service across **one or more hosts** (e.g. `app1..app6`) — maps to `./properties.sh`: preview / update / add_csv / remove_csv / rename / append / insert / diff / restore. One service at a time, run per selected host. Append & insert take a **pasted block**, staged to a file on the server (a previous one removed, a fresh one created) before the wrapper runs. `--test` edits a copy and shows the diff without touching the live file. |

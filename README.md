...
| **Stg Deployment** | The **staging** channel (`/stg-deployment`), backed by a wholly separate backend **container** (`deployment-automation-backend-stg`). Upload-driven: upload a jar / `application.properties` / UI tarball, the service stages it into the `stg-deployment` bundle on the jump host, then runs `./run.sh` (core/portal) or `portalui/run.sh` live. The review shows each file's **SHA-256** (computed in the browser) before anything is uploaded. |
| **Application Properties** | Surgical, per-`host:app` edits of `application.properties` (maps to `./properties.sh`): preview / update / add_csv / remove_csv / rename / append / insert / diff / restore. Append & insert take a **pasted block**, staged to a file on the server (a previous one removed, a fresh one created) before the wrapper runs. `--test` edits a copy and shows the diff without touching the live file. |
